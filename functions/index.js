...
exports.searchLawFunction = onRequest(async (req, res) => {
  ...
  if (Object.keys(LawInfo).length) {
    lawFilterName = Object.keys(LawInfo).filter(key => {
      return (
        LawInfo[key]['lawDescription'].match(new RegExp(`${input}`, 'gim')) ||
        LawInfo[key]['lawNumber'].match(new RegExp(`${input}`, 'gim')) ||
        LawInfo[key]['lawNameDisplay'].match(new RegExp(`${input}`, 'gim'))
      );
    ...
    if (input.match(/(\w+|\(|\)|\.|\+|\-|\,|\&|\?|\;|\!|\/)/gim)) {
      function a(key, key1) {
        ...
        Object.values(key1)[0].map((key2, i1) => {
          ...
          let article = Object.keys(key2)[0].replace(
            /(?<=\w*)\\(?=\w*)/gim,
            '/',
          );
          let point = Object.values(key2)[0].replace(
            /(?<=\w*)\\(?=\w*)/gim,
            '/',
          );
      ...
      Object.keys(data).map((key, i) => {
        ...
        data[key].map((key1, i1) => {
          ...
          if (Object.keys(key1)[0].match(/^phần thứ .*/gim)) {
            ...
            if (
              Object.keys(Object.values(key1)[0][0])[0].match(/^Chương .*/gim)
            ) {
